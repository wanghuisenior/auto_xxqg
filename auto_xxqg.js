...
function checkAndUpdate(question, ansFromDB, correct_answer) {
    if (className("Button").text("下一题").exists() || className("Button").text("完成").exists()) {//答错了
        try{
        ...
        while (nCout < 5) {
            if (textStartsWith("正确答案").exists()) {
                var correctAns = textStartsWith("正确答案").findOnce().text().substr(5);
